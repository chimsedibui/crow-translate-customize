...
def test_status_line_names_the_preferred_provider(qapp, tmp_path):
    ...
    runner = AsyncLoopRunner()
    try:
        manager = ProviderManager([FakeProvider(), Azure()], preferred="azure-openai")
        vm = TranslationViewModel(
            manager, AppSettings(), SettingsStore(tmp_path / "s.toml"), HistoryStore(tmp_path / "h.json"), runner
        )
