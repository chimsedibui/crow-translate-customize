...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_googleCloudTranslator(new GoogleCloudTranslator(this))
...
void MainWindow::requestTranslation()
{
    ...
    if (useGoogleCloud())
        m_googleCloudTranslator->translate(ui->sourceEdit->toSourceText(), translationLang, ui->sourceLanguagesWidget->checkedLanguage());
    else
...
{
    if (useGoogleCloud()) {
        const QOnlineTranslator::Language translationLang = preferredTranslationLanguage(m_googleCloudTranslator->sourceLanguage());
        m_googleCloudTranslator->translate(ui->sourceEdit->toSourceText(), translationLang, m_googleCloudTranslator->sourceLanguage());
        return;
    }
...
{
    const bool parsed = useGoogleCloud() ? ui->translationEdit->parseTranslationData(m_googleCloudTranslator) : ui->translationEdit->parseTranslationData(m_translator);
    if (!parsed) {
    ...
    const QOnlineTranslator::Language sourceLang = useGoogleCloud() ? m_googleCloudTranslator->sourceLanguage() : m_translator->sourceLanguage();
    const QOnlineTranslator::Language translationLang = useGoogleCloud() ? m_googleCloudTranslator->translationLanguage() : m_translator->translationLanguage();

    // Display languages on "Auto" buttons
    if (ui->sourceLanguagesWidget->isAutoButtonChecked())
        ui->sourceLanguagesWidget->setAutoLanguage(sourceLang);

    if (ui->translationLanguagesWidget->isAutoButtonChecked())
        ui->translationLanguagesWidget->setAutoLanguage(translationLang);
    else
        ui->translationLanguagesWidget->setAutoLanguage(QOnlineTranslator::Auto);
...
{
    if (useGoogleCloud())
        m_googleCloudTranslator->detectLanguage(ui->sourceEdit->toSourceText());
    else
...
{
    if (useGoogleCloud()) {
        if (m_googleCloudTranslator->error() != QOnlineTranslator::NoError) {
            QMessageBox::critical(this, tr("Unable to detect language"), m_googleCloudTranslator->errorString());
            return;
        }
        ui->sourceLanguagesWidget->setAutoLanguage(m_googleCloudTranslator->sourceLanguage());
        return;
    }
...
{
    const QOnlineTranslator::TranslationError translationError = useGoogleCloud() ? m_googleCloudTranslator->error() : m_translator->error();
    if (translationError != QOnlineTranslator::NoError) {
        const QString errorText = useGoogleCloud() ? m_googleCloudTranslator->errorString() : m_translator->errorString();
        QMessageBox::critical(this, tr("Unable to translate text"), errorText);
        return;
    }
...
void MainWindow::buildTranslationState(QState *state) const
{
    ...
    connect(abortPreviousState, &QState::entered, m_googleCloudTranslator, &GoogleCloudTranslator::abort);
    ...
    // Only one of m_translator/m_googleCloudTranslator is actually asked to translate per
    // request (decided in requestTranslation()/requestRetranslation(), see useGoogleCloud());
    // both sets of transitions are wired unconditionally so whichever one is used can drive
    // the state machine, since the engine choice can change at runtime.
    auto *translationRunningTransition = new TranslatorAbortedTransition<QOnlineTranslator>(m_translator, abortPreviousState);
    translationRunningTransition->setTargetState(requestState);
    auto *googleCloudTranslationRunningTransition = new TranslatorAbortedTransition<GoogleCloudTranslator>(m_googleCloudTranslator, abortPreviousState);
    googleCloudTranslationRunningTransition->setTargetState(requestState);

    auto *otherLangTransition = new RetranslationTransition<QOnlineTranslator>(m_translator, ui->translationLanguagesWidget, checkLanguagesState);
    otherLangTransition->setTargetState(requestInOtherLangState);
    auto *googleCloudOtherLangTransition = new RetranslationTransition<GoogleCloudTranslator>(m_googleCloudTranslator, ui->translationLanguagesWidget, checkLanguagesState);
    googleCloudOtherLangTransition->setTargetState(requestInOtherLangState);

    requestState->addTransition(m_translator, &QOnlineTranslator::finished, checkLanguagesState);
    requestState->addTransition(m_googleCloudTranslator, &GoogleCloudTranslator::finished, checkLanguagesState);
    ...
    requestInOtherLangState->addTransition(m_googleCloudTranslator, &GoogleCloudTranslator::finished, parseState);
}
...
void MainWindow::buildSpeakSourceState(QState *state) const
{
    ...
    auto *googleCloudTranslationRunningTransition = new TranslatorAbortedTransition<GoogleCloudTranslator>(m_googleCloudTranslator, abortPreviousState);
    googleCloudTranslationRunningTransition->setTargetState(requestLangState);

    auto *errorTransition = new TranslatorErrorTransition<QOnlineTranslator>(m_translator, parseLangState);
    errorTransition->setTargetState(m_stateMachine->initialState());
    auto *googleCloudErrorTransition = new TranslatorErrorTransition<GoogleCloudTranslator>(m_googleCloudTranslator, parseLangState);
    googleCloudErrorTransition->setTargetState(m_stateMachine->initialState());
    ...
    requestLangState->addTransition(m_googleCloudTranslator, &GoogleCloudTranslator::finished, parseLangState);
...
bool MainWindow::useGoogleCloud() const
{
    return currentEngine() == QOnlineTranslator::Google && GoogleCloudTranslator::isConfigured();
}
