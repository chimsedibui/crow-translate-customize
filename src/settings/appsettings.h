...
public:
    ...
    // Google Cloud Translation API (official REST API, used instead of QOnlineTranslator's
    // scraped Google engine when a key is configured, see GoogleCloudTranslator)
    QByteArray googleCloudApiKey() const;
    void setGoogleCloudApiKey(const QByteArray &apiKey);
    static QByteArray defaultGoogleCloudApiKey();
