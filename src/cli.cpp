...
void Cli::process(const QCoreApplication &app)
{
    ...
    if (parser.value(engine) == QLatin1String("google")) {
        ...
        m_useGoogleCloud = GoogleCloudTranslator::isConfigured();
...
void Cli::requestTranslation()
{
    ...
    if (m_useGoogleCloud)
        m_googleCloudTranslator->translate(m_sourceText, translationLang, m_sourceLang);
    else
...
{
    const QOnlineTranslator::TranslationError translationError = m_useGoogleCloud ? m_googleCloudTranslator->error() : m_translator->error();
    if (translationError != QOnlineTranslator::NoError) {
        const QString errorText = m_useGoogleCloud ? m_googleCloudTranslator->errorString() : m_translator->errorString();
        qCritical() << tr("Error: %1").arg(errorText);
        m_stateMachine->stop();
        return;
    }

    if (m_sourceLang == QOnlineTranslator::Auto)
        m_sourceLang = m_useGoogleCloud ? m_googleCloudTranslator->sourceLanguage() : m_translator->sourceLanguage();
}

template<typename Translator>
void Cli::printTranslation(Translator *translator)
{
    // JSON mode
    if (m_json) {
        m_stdout << translator->toJson().toJson();
        return;
    }
    ...
    if (m_brief) {
        ...
        m_stdout << translator->translation() << Qt::endl;
#else
        m_stdout << translator->translation() << endl;
#endif
        return;
    }
    ...
    if (!m_sourcePrinted) {
        m_stdout << translator->source() << '\n';
        if (!translator->sourceTranslit().isEmpty())
            m_stdout << '(' << translator->sourceTranslit().replace('\n', QStringLiteral(")\n(")) << ")\n";
        m_sourcePrinted = true;
    }
    m_stdout << '\n';

    // Languages
    m_stdout << "[ " << translator->sourceLanguageName() << " -> ";
    m_stdout << translator->translationLanguageName() << " ]\n\n";

    // Translation and its transliteration
    if (!translator->translation().isEmpty()) {
        m_stdout << translator->translation() << '\n';
        if (!translator->translationTranslit().isEmpty())
            m_stdout << '/' << translator->translationTranslit().replace('\n', QStringLiteral("/\n/")) << "/\n";
        m_stdout << '\n';
    }

    // Translation options
    if (!translator->translationOptions().isEmpty()) {
        m_stdout << tr("%1 - translation options:").arg(translator->source()) << '\n';
    ...
    if (!translator->examples().isEmpty()) {
        m_stdout << tr("%1 - examples:").arg(translator->source()) << '\n';
...
{
    if (m_useGoogleCloud)
        printTranslation(m_googleCloudTranslator);
    else
        printTranslation(m_translator);
}

void Cli::requestLanguage()
{
    if (m_useGoogleCloud)
        m_googleCloudTranslator->detectLanguage(m_sourceText);
    else
...
{
    if (m_useGoogleCloud)
        speak(m_googleCloudTranslator->translation(), m_googleCloudTranslator->translationLanguage());
    else
...
void Cli::buildTranslationStateMachine()
{
    ...
    for (QOnlineTranslator::Language lang : qAsConst(m_translationLanguages)) {
        ...
        if (m_useGoogleCloud)
            requestTranslationState->addTransition(m_googleCloudTranslator, &GoogleCloudTranslator::finished, parseDataState);
        else
